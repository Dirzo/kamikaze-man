(()=>{
  ...
  const BUILD='cwl-presentation-proof-v1-20260915a';
  function read(){
    ...
    if(root){root.dataset.cwlPresentation=ok?'pass':'fail';root.dataset.cwlPlayerScale=scale?String(scale):'0';root.dataset.cwlCompactHud=hud?'ready':'missing';root.dataset.cwlPresentationDesktop=desktop?'yes':'no'}
