(()=>{
  ...
  const BUILD='cwl-parking-proof-v2-20260915b';
  ...
  function run(){
    ...
    if(!d?.start||!d?.setZone||!d?.spawnPack||!d?.clearPack||!d?.draw||!d?.skill||!api?.state||!api?.hit){setTimeout(run,90);return}
    try{
      const old=Number(api.state()?.zoneI??1);
      d.start();d.setZone(2);try{d.clearPack()}catch(_){}d.spawnPack();
      ...
      state.ok=bg&&enemy&&art&&renderer&&heavy&&damage&&skill&&kill&&secondPack&&native&&body;
      Object.assign(state,{bg,enemy,art,renderer,heavy,damage,skill,kill,secondPack,native,body,firstTypes,secondTypes:[...new Set(second.map(e=>e.type))],enemyCount:second.length,zoneName:s.zoneName||'',weapon:s.weapon?.heavyBaseId||'',at:Date.now()});
      root.dataset.cwlParkingProof=state.ok?'pass':'fail';
      root.dataset.cwlParkingPlayable=state.ok?'pass':'fail';
    ...
    }catch(e){state.error=String(e?.message||e);root.dataset.cwlParkingProof='fail';root.dataset.cwlParkingPlayable='fail';globalThis.__CWL_PARKING_PROOF=state}
  }
