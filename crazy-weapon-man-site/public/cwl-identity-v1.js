(()=>{
  ...
  const BUILD='cwl-identity-v1-20260915a';
  const replaceText=s=>String(s??'')
    .replaceAll('CRAZY WEAPON MAN JR.','CRAZY WEAPON LADY JR.')
    .replaceAll('Crazy Weapon Man Jr.','Crazy Weapon Lady Jr.')
  ...
  function patchDOM(root=document.body){
    ...
    while(walker.nextNode()){
      ...
      if(/Crazy Weapon Man|CRAZY WEAPON MAN/.test(n.nodeValue||''))touched.push(n);
    }
    ...
    for(const el of root.querySelectorAll?.('[aria-label],[title],[placeholder]')||[]){
      for(const a of ['aria-label','title','placeholder']){
        const v=el.getAttribute(a);if(v&&/Crazy Weapon Man|CRAZY WEAPON MAN/.test(v))el.setAttribute(a,replaceText(v));
      }
    }
  }
  ...
  const mo=new MutationObserver(ms=>{
    for(const m of ms)for(const n of m.addedNodes){
      if(n.nodeType===Node.TEXT_NODE){if(/Crazy Weapon Man|CRAZY WEAPON MAN/.test(n.nodeValue||''))n.nodeValue=replaceText(n.nodeValue)}
