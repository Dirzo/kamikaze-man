(()=>{
  ...
  const BUILD='cwl-enemy-telegraphs-v2-20260915a';
  const state={build:BUILD,installed:false,waitingFor:'enemy renderer',draws:0,lastType:'',lastState:'',installedAt:0};
  const TAU=Math.PI*2;
  ...
  function before(ctx,d){
    ...
    if(d.kind==='cast'){ring(X,cx,bt,26+7*p,7+2*p,d.col,.38+.22*p,2.2);line(X,cx,e.y+e.h*.45,pcx,pcy,d.col,1.8,.28+.22*p)}
    else if(d.kind==='charge'){const dir=e.dir||Math.sign(pcx-cx)||1;wedge(X,cx,bt-6,dir,105+20*p,26,d.col,.15+.13*p);line(X,cx,bt-7,cx+dir*(115+20*p),bt-7,d.col,2,.44+.18*p)}
    else if(d.kind==='blink'){ring(X,pcx,Math.min(bt,pcy+28),30+8*p,8+2*p,d.col,.38+.24*p,2.4);cross(X,pcx,pcy,d.col,.46+.28*p)}
    else if(d.kind==='blast'){const r=54+10*p;ring(X,cx,bt-4,r,r*.24,d.col,.46+.26*p,3);ring(X,cx,e.y+e.h*.45,22+5*p,22+5*p,d.col,.20+.16*p,2)}
    X.restore();
  }
  function after(ctx,d){if(!d)return;const {X,e}=ctx,cx=e.x+e.w/2,y=e.y-10,p=pulse();X.save();X.globalAlpha=.72+.2*p;X.fillStyle=d.col;X.shadowColor=d.col;X.shadowBlur=9;X.beginPath();X.moveTo(cx,y-10);X.lineTo(cx+7,y+3);X.lineTo(cx-7,y+3);X.closePath();X.fill();X.restore();state.draws++;state.lastType=e.type||'';state.lastState=e.state||''}
  function install(){
    const hook=globalThis.__CWM_RENDER_HOOK;
    if(!hook?.drawEnemy){state.waitingFor='enemy renderer';return false}
    if(hook.drawEnemy.__cwlTelegraph){state.installed=true;state.waitingFor='';return true}
    ...
    wrapped.__cwlTelegraph=true;wrapped.__cwlTelegraphBase=base;
    globalThis.__CWM_RENDER_HOOK={...hook,__cwlTelegraphsV2:true,drawEnemy:wrapped};
    state.installed=true;state.waitingFor='';state.installedAt=Date.now();
    document.documentElement.dataset.cwlEnemyTelegraphs='ready';
    console.info('Crazy Weapon Lady enemy telegraphs installed',BUILD);
    return true;
  }
  let tries=0;const timer=setInterval(()=>{tries++;if(install()||tries>50){clearInterval(timer);if(!state.installed){state.waitingFor='enemy renderer unavailable';console.warn('CWL enemy telegraphs could not attach',state)}}},80);install();
