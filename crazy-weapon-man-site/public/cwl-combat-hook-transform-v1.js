(()=>{
  ...
  const BUILD='cwl-combat-hook-transform-v1-20260915a';
  const helper=`
...
`;
  ...
  function transform(input){
    ...
    function patchSegment(name,nextName,patcher){
      const start=html.indexOf('function '+name+'(');if(start<0){report.warnings.push('missing '+name);return 0}
      const end=nextName?html.indexOf('\nfunction '+nextName+'(',start+10):html.indexOf('\nfunction ',start+10);
      const stop=end>=0?end:html.length;let seg=html.slice(start,stop),next=patcher(seg);
      if(next===seg){report.warnings.push('unable to patch '+name);return 0}
      html=html.slice(0,start)+next+html.slice(stop);return 1
    }

    report.patches.hit=patchSegment('hitE','crit',seg=>seg.replace(
      /if\(e\.hp<=0\)killE\(e,n>=before,o\.sourceMod\|\|null,o\.sourceDam\|\|n\);return n\s*$/,
    ...
    report.patches.kill=patchSegment('killE','checkLevel',seg=>seg.replace(
      /checkLevel\(\)\s*}\s*$/,
      "checkLevel();__cwlCombatEmit('kill',{enemy:e,oneShot:one,sourceMod,sourceDamage:sd,weapon:w,time})}"
    ));

    report.patches.skill=patchSegment('skill',null,seg=>seg.replace(
      /proceduralProc\(w\)\s*}\s*$/,
      "proceduralProc(w);__cwlCombatEmit('skill',{weapon:w,cost,cooldown:pl.scd,mana:pl.mana,time})}"
    ));
