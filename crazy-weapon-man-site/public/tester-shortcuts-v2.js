(()=>{
  ...
  const BUILD='tester-warp-20260914b';
  let panel=null,zoneText=null,rendererText=null,bridgeText=null,drawText=null;
  let cornerTaps=[];
  ...
  globalThis.CWM_TESTER={build:BUILD,show:()=>{ensurePanel().classList.add('show');updatePanel()},hide:()=>panel?.classList.remove('show'),warp:cleanWarp,sewer:()=>cleanWarp(1),next:()=>nextZone(1),prev:()=>nextZone(-1),status:()=>({zone:currentZone(),name:zoneName(currentZone()),renderer:globalThis.CWM_V2_RENDERER||null,bridge:globalThis.__CWM_NATIVE_RENDER_BRIDGE||null,lastDraw:globalThis.__CWM_V2_LAST_DRAW||null})};
