(()=>{
  ...
  const BUILD='tester-warp-20260915e';
  let panel=null,zoneText=null,rendererText=null,bridgeText=null,drawText=null,spriteText=null,heavyText=null;
  let cornerTaps=[];
  ...
  function updatePanel(){
    ...
    const lady=globalThis.CWL_HEAVY_BODY,heavy=globalThis.CWM_HEAVY_ONLY,atlas=globalThis.CWM_HEAVY_RUNTIME_ATLAS,weapon=heavy?.state?.()?.weapon;
    ...
    if(spriteText){const ready=!!sprites?.ready,err=sprites?.playerError||sprites?.enemyError;const lens=sprites?` • ${Math.round((sprites.playerBytes||0)/1024)}k/${Math.round((sprites.enemyBytes||0)/1024)}k`:'';spriteText.textContent=sprites?`LEGACY SPRITES: ${ready?'READY':err?'ERROR':'LOADING'} • P:${sprites.playerReady?'OK':'…'} E:${sprites.enemyReady?'OK':'…'}${lens} • ${sprites.build||''}${err?' • '+err:''}`:'LEGACY SPRITES: NOT LOADED';spriteText.style.color=ready?'#79ff9c':err?'#ff667a':'#ffb86b'}
    if(heavyText){
      const bodyReady=!!lady?.state?.ready,bodyErr=lady?.state?.error,chunks=lady?.state?.transportChunks||0,heavyOn=!!heavy?.enabled,stats=atlas?.stats?.();
      const base=weapon?.heavyLabel||weapon?.heavyBaseId||'—',frame=lady?.state?.frame||'—';
      heavyText.textContent=`CWL HEAVY: ${bodyReady&&heavyOn?'READY':bodyErr?'ERROR':'LOADING'} • BODY ${chunks}/3 • ${base} • FRAME ${frame}${stats?` • ATLAS ${stats.entries}/${stats.capacity}`:''}${bodyErr?' • '+bodyErr:''}`;
      heavyText.style.color=bodyReady&&heavyOn?'#79ff9c':bodyErr?'#ff667a':'#ffb86b';
    }
  ...
  globalThis.CWM_TESTER={build:BUILD,show:()=>{ensurePanel().classList.add('show');updatePanel()},hide:()=>panel?.classList.remove('show'),warp:cleanWarp,sewer:()=>cleanWarp(1),next:()=>nextZone(1),prev:()=>nextZone(-1),status:()=>({zone:currentZone(),name:zoneName(currentZone()),renderer:globalThis.CWM_V2_RENDERER||null,bridge:globalThis.__CWM_NATIVE_RENDER_BRIDGE||null,lastDraw:globalThis.__CWM_V2_LAST_DRAW||null,sprites:globalThis.CWM_V2_SPRITES||null,lady:globalThis.CWL_HEAVY_BODY?.state||null,heavy:globalThis.CWM_HEAVY_ONLY?.state?.()||null,atlas:globalThis.CWM_HEAVY_RUNTIME_ATLAS?.stats?.()||null})};
